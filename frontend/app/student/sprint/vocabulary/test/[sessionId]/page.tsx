...
export default function SprintVocabularyTestPage() {
  ...
  return (
    <main className="min-h-screen bg-[#F4F7F6] pb-32">
      <header className="sticky top-0 z-10 border-b border-[#DFE9E5] bg-[#F4F7F6]/95 px-5 py-4 backdrop-blur sm:px-6">
        <div className="mx-auto max-w-[680px] md:max-w-[760px] lg:max-w-[820px]">
          <div className="flex items-center justify-between">
            <button onClick={() => router.push("/student/sprint/vocabulary")} className="text-sm font-black text-[#64748B]">챌린지 홈</button>
            <span className={`rounded-full px-3 py-1 text-xs font-bold ${saveState === "저장 실패" ? "bg-red-50 text-red-500" : "bg-[#E7F9F2] text-[#19A879]"}`}>{saveState}</span>
          </div>
          <div className="mt-3 flex items-end justify-between gap-4">
            <div><p className="text-xs font-black text-[#19A879]">{session.session_type === "review" ? "WRONG NOTE RETRY" : "SPRINT VOCAB TEST"}</p><h1 className="mt-1 break-keep text-2xl font-black text-[#17213B]">{session.challenge_name}</h1></div>
            <p className="shrink-0 text-sm font-black text-[#17213B]">{answered}/{session.total_count}</p>
          </div>
      ...
      <div className="mx-auto max-w-[680px] px-5 pt-6 sm:px-6 md:max-w-[760px] lg:max-w-[820px]">
        {error && <p className="mb-4 rounded-2xl bg-red-50 px-4 py-3 text-sm font-bold text-red-600">{error}</p>}
        <div className="mb-5 flex items-center justify-between gap-3 rounded-[22px] bg-[#E7F9F2] px-4 py-3 ring-1 ring-[#CBEFE1]"><p className="break-keep text-sm font-bold text-[#276B58]">영어 단어를 보고 한국어 뜻을 입력하세요.</p><span className="shrink-0 rounded-full bg-white/70 px-2.5 py-1 text-xs font-black text-[#C46731]">빈칸 {empty}</span></div>
        <div className="space-y-3 md:space-y-4">
          {session.questions.map((question, index) => (
            <section key={question.id} className="rounded-[24px] bg-white p-5 shadow-[0_10px_28px_rgba(23,33,59,.06)] ring-1 ring-[#E2EAE7] md:p-6">
              <div className="flex items-baseline gap-3"><span className="text-xs font-black text-[#A0AAAF]">{String(index + 1).padStart(2, "0")}</span><h2 className="break-words text-2xl font-black tracking-tight text-[#17213B]">{question.english}</h2></div>
              <input ref={(element) => { inputs.current[index] = element; }} value={values[question.id] ?? ""} onChange={(event) => setValues({ ...values, [question.id]: event.target.value })} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); inputs.current[index + 1]?.focus(); } }} autoComplete="off" placeholder="한국어 뜻 입력" className="mt-4 h-14 w-full rounded-2xl border-2 border-[#E4E9EC] bg-[#FAFCFB] px-4 text-lg font-bold text-[#17213B] outline-none transition focus:border-[#45D3A2] focus:bg-white focus:ring-4 focus:ring-[#E7F9F2]" />
            </section>
          ))}
        </div>
        <button disabled={submitting} onClick={() => void submit()} className="mt-7 h-14 w-full rounded-[20px] bg-[#17213B] text-base font-black text-white shadow-[0_14px_32px_rgba(23,33,59,.18)] transition active:scale-[.99] disabled:opacity-50">{submitting ? "채점 중..." : "답안 제출하고 채점하기"}</button>
      </div>
    </main>
  );
}
