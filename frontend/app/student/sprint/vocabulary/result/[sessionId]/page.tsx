...
export default function SprintVocabularyResultPage() {
  ...
  return (
    <main className="min-h-screen bg-[#F4F7F6] pb-36">
      <div className="mx-auto max-w-[680px] px-5 py-7 sm:px-6 md:max-w-[760px] lg:max-w-[820px]">
        ...
        <section className="mt-5 overflow-hidden rounded-[30px] bg-[#17213B] p-6 text-white shadow-[0_18px_36px_rgba(23,33,59,.20)]">
          <p className="text-sm font-bold text-[#65E6BA]">{result.session_type === "review" ? "오답 재시험 결과" : "오늘의 채점 결과"}</p>
          <div className="mt-3 flex items-end justify-between gap-4">
            ...
            <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-full border-[7px] border-[#65E6BA] text-lg font-black">{rate}%</div>
          </div>
        </section>
        {error && <p className="mt-4 rounded-2xl bg-red-50 px-4 py-3 text-sm font-bold text-red-600">{error}</p>}
        <div className="mt-4 grid grid-cols-2 gap-3"><Link href="/student/sprint/vocabulary/wrong-notes" className="rounded-2xl bg-white p-4 text-center text-sm font-black text-[#19A879] shadow-[0_8px_20px_rgba(23,33,59,.06)] ring-1 ring-[#E2EAE7]">오답노트 보기</Link><button disabled={starting} onClick={() => void review()} className="rounded-2xl bg-[#FFF1C9] p-4 text-sm font-black text-[#9A6500] shadow-[0_8px_20px_rgba(23,33,59,.06)] ring-1 ring-[#F5DE9D] disabled:opacity-50">{starting ? "준비 중..." : "오답 재시험"}</button></div>
        <div className="mt-7 flex items-center justify-between gap-3">
          <h2 className="text-xl font-black text-[#17213B]">문항별 결과</h2>
          <div className="flex shrink-0 rounded-full bg-[#E7EBF0] p-1">
            <button
        ...
        <div className="mt-3 space-y-3 md:space-y-4">
          {questions.map((question) => (
            <article key={question.id} className={`rounded-[24px] bg-white p-5 shadow-[0_10px_28px_rgba(23,33,59,.06)] ring-1 ${question.is_correct ? "ring-[#DCEFE7]" : "ring-[#F4D7D1]"}`}>
              <div className="flex items-center justify-between gap-3">
                <div className="flex min-w-0 items-baseline gap-2">
                  ...
                  <h3 className="break-words text-xl font-black text-[#17213B]">{question.english}</h3>
                </div>
              ...
              <p className="mt-3 break-words rounded-2xl bg-[#F0FAF6] px-3 py-2 text-[13px] font-bold text-[#276B58]">{question.accepted_answers.join(" · ")}</p>
              <div className="mt-2 flex items-baseline gap-1.5 text-[13px]">
                <span className="font-medium text-[#98A2B3]">내 답안</span>
                <span className={`break-words font-bold ${question.input_answer ? (question.is_correct ? "text-[#12815F]" : "text-[#E15B45]") : "text-[#98A2B3]"}`}>
