...
export default function VocabularyChallengesPage() {
  ...
  const filteredChallenges = challenges.filter((challenge) => (
    statusFilter === "all" ? true : statusFilter === "active" ? challenge.is_active : !challenge.is_active
  ));

  const updateWordBankRange = (patch: Partial<typeof form>) => {
    const next = { ...form, ...patch };
    if (next.source_type === "word_bank") {
      const start = Number(next.start_bank_day || 1);
      const end = Number(next.end_bank_day || start);
      const daysPerLearningDay = Math.max(1, Number(next.bank_days_per_learning_day || 1));
      const learningDays = Math.max(1, Math.ceil((Math.abs(end - start) + 1) / daysPerLearningDay));
      const startDate = new Date(`${next.start_date}T00:00:00`);
      if (!Number.isNaN(startDate.getTime())) {
        startDate.setDate(startDate.getDate() + learningDays - 1);
        next.end_date = startDate.toISOString().slice(0, 10);
      }
    }
    setForm(next);
  };
  ...
  const load = async () => {
    const requestedStudentId = typeof window !== "undefined" ? new URLSearchParams(window.location.search).get("student_id") ?? "" : "";
    ...
    setForm((value) => ({
      ...value,
      student_id: value.student_id || requestedStudentId || (studentRows[0] ? String(studentRows[0].id) : ""),
      ...
      <div className="mx-auto max-w-[1100px] px-5 py-8">
        <div className="mb-7 flex items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Link href="/admin/vocabulary-review" className="rounded-full bg-[#17213B] px-4 py-2 text-sm font-black text-white shadow-sm">오답 통합 검토</Link>
        ...
        <div className="grid gap-5 lg:grid-cols-[400px_1fr]">
          <form onSubmit={createChallenge} className="h-fit rounded-[28px] bg-[#122238] p-6 text-white shadow-xl">
            ...
            <div className="mt-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="text-xs font-bold text-white/60">시작일<input type="date" required value={form.start_date} onChange={(event) => updateWordBankRange({ start_date: event.target.value })} className="mt-1.5 h-12 w-full rounded-2xl border-0 bg-white px-3 text-[#17213B]" /></label>
                ...
                <>
                  <label className="block text-xs font-bold text-white/60">워드뱅크<select required value={form.word_bank_id} onChange={(event) => {
                    ...
                    setForm({
                      ...
                      end_bank_day: form.bank_day_direction === "descending" ? "1" : String(nextBank?.total_days ?? 50),
                    });
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <label className="text-xs font-bold text-white/60">시작 bank DAY<input type="number" min="1" max={selectedBank?.total_days ?? 365} value={form.start_bank_day} onChange={(event) => updateWordBankRange({ start_bank_day: event.target.value })} className="mt-1.5 h-12 w-full rounded-2xl border-0 bg-white px-4 text-[#17213B]" /></label>
                    <label className="text-xs font-bold text-white/60">종료 bank DAY<input type="number" min="1" max={selectedBank?.total_days ?? 365} value={form.end_bank_day} onChange={(event) => updateWordBankRange({ end_bank_day: event.target.value })} className="mt-1.5 h-12 w-full rounded-2xl border-0 bg-white px-4 text-[#17213B]" /></label>
                    <label className="text-xs font-bold text-white/60">진행 방향<select value={form.bank_day_direction} onChange={(event) => updateWordBankRange({ bank_day_direction: event.target.value, start_bank_day: event.target.value === "descending" ? String(selectedBank?.total_days ?? 1) : "1", end_bank_day: event.target.value === "descending" ? "1" : String(selectedBank?.total_days ?? 50) })} className="mt-1.5 h-12 w-full rounded-2xl border-0 bg-[#203249] px-4 text-white"><option value="ascending">정방향</option><option value="descending">역방향</option></select></label>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
          ...
          <section className="space-y-3">
            <div className="grid grid-cols-3 rounded-2xl bg-white p-1 shadow-sm">
              {([["active", "진행 중"], ["completed", "완료"], ["all", "전체"]] as const).map(([value, label]) => (
                <button key={value} type="button" onClick={() => setStatusFilter(value)} className={`rounded-xl py-2.5 text-xs font-black ${statusFilter === value ? "bg-[#17213B] text-white" : "text-[#8A94A8]"}`}>{label}</button>
              ))}
            </div>
            {filteredChallenges.length === 0 && <div className="rounded-[28px] bg-white p-10 text-center text-sm font-bold text-[#98A2B3]">해당 상태의 챌린지가 없습니다.</div>}
