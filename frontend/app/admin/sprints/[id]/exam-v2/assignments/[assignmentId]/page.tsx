        ...
        <>
            ...
            <div className="rounded-lg border border-[#DFE7F0] bg-white p-5 shadow-sm">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-xs font-bold text-[#8290A6]">시험일 {detail.exam.exam_date || "-"} · 응시 한도 {detail.assignment.attempt_limit}회</p>
                </div>
          ...
          <section className="mt-5 overflow-hidden rounded-lg border border-[#DFE7F0] bg-white shadow-sm">
            <div className="border-b border-[#E8EDF3] px-5 py-4">
              <h2 className="text-sm font-black text-[#17213B]">Attempt 목록</h2>
            </div>
                  ...
                  <tbody className="divide-y divide-[#EDF1F5]">
                    {detail.attempts.map((attempt) => (
                      <tr key={attempt.id} className={selectedAttemptId === attempt.id ? "bg-[#F2F8FF]" : ""}>
                        <td className="px-4 py-3 font-black text-[#17213B]">#{attempt.attempt_no}</td>
          ...
          {!attemptLoading && attemptDetail && (
            <section className="mt-5 overflow-hidden rounded-lg border border-[#DFE7F0] bg-white shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#E8EDF3] px-5 py-4">
                <div>
                  <h2 className="text-sm font-black text-[#17213B]">Attempt #{attemptDetail.attempt.attempt_no} 답안</h2>
              ...
              <p className="border-t border-[#E8EDF3] bg-[#FAFCFE] px-5 py-3 text-xs font-semibold text-[#8290A6]">
                채점·결과 공개·재응시 승인 작업 버튼은 관리자 프론트 3차 범위에서 기존 API에 연결합니다.
              </p>
            </section>
          )}
        </>
